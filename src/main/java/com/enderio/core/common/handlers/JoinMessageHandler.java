...
public class JoinMessageHandler {
  ...
  public void onPlayerJoin(PlayerLoggedInEvent event) {
    if(ConfigHandler.invisibleMode == 1 && ConfigHandler.instance().showInvisibleWarning()) {
      ...
      event.player.addChatMessage(new TextComponentString(text1));
      event.player.addChatMessage(new TextComponentString(text2));
      if(EnderCore.instance.invisibilityRequested()) {
        event.player.addChatMessage(new TextComponentString(text3));
      }
      event.player.addChatMessage(new TextComponentString(text4));
    }
  }
}
