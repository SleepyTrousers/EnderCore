...
public class VecmathUtil {
    ...
    /**
     * Compute the plane equation <code>Ax + By + Cz + D = 0</code> for the plane defined by the three points which lie
     * on the plane, a, b, and c, and placing the result into r. The plane equation can be summarised as the normal
     * vector of the plane (A,B,C) and the distance to the plane from the origin (D).
     * 
     * @param a      vector a.
     * @param b      vector b.
     * @param c      vector c.
    ...
    public static Matrix4d createProjectionMatrixAsPerspective(double left, double right, double bottom, double top,
        ...
        Matrix4d matrix = new Matrix4d();
        matrix.set(
                new double[] { 2.0 * zNear / (right - left), 0.0, 0.0, 0.0, 0.0, 2.0 * zNear / (top - bottom), 0.0, 0.0,
                        A, B, C, -1.0, 0.0, 0.0, D, 0.0
    ...
    public static Matrix4d createProjectionMatrixAsOrtho(double left, double right, double bottom, double top,
        ...
        matrix.set(
                new double[] { 2d / (right - left), 0, 0, tx, 0, 2d / (top - bottom), 0, ty, 0, 0, -2d / (far - near),
                        tz, 0, 0, 0, 1 });
    ...
    public static Vector3d preMultiply(Vector3d v, Matrix4d mat) {
        ...
        double x = (m.getElement(0, 0) * v.x + m.getElement(1, 0) * v.y + m.getElement(2, 0) * v.z + m.getElement(3, 0))
                * d;
        double y = (m.getElement(0, 1) * v.x + m.getElement(1, 1) * v.y + m.getElement(2, 1) * v.z + m.getElement(3, 1))
                * d;
        double z = (m.getElement(0, 2) * v.x + m.getElement(1, 2) * v.y
                + m.getElement(2, 2) * v.z
                + m.getElement(3, 2) * d);
