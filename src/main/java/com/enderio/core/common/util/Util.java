...
public class Util {
    ...
    public static BlockCoord canPlaceItem(ItemStack itemUsed, Block blockIdToBePlaced, EntityPlayer player, World world,
    ...
    public static EntityItem createDrop(World world, ItemStack stack, double x, double y, double z,
    ...
    public static List<MovingObjectPosition> raytraceAll(World world, Vec3 startVec, Vec3 endVec,
            boolean includeLiquids) {
        ...
        if (!Double.isNaN(startVec.xCoord) && !Double.isNaN(startVec.yCoord) && !Double.isNaN(startVec.zCoord)) {
            if (!Double.isNaN(endVec.xCoord) && !Double.isNaN(endVec.yCoord) && !Double.isNaN(endVec.zCoord)) {
                ...
                if ((!p_147447_4_ || block.getCollisionBoundingBoxFromPool(world, l, i1, j1) != null)
                        && block.canCollideCheck(k1, includeLiquids)) {
                    MovingObjectPosition movingobjectposition = block
                            .collisionRayTrace(world, l, i1, j1, startVec, endVec);
                ...
                while (k1-- >= 0) {
                    if (Double.isNaN(startVec.xCoord) || Double.isNaN(startVec.yCoord)
