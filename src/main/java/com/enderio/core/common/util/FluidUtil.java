...
public class FluidUtil {
    ...
    static {
        try {
            ...
            if (Loader.isModLoaded("BuildCraft|Transport")) {
                Log.warn(
                        "ItemUtil: Could not register Build Craft pipe handler. Fluid conduits will show connections to all Build Craft pipes.");
        ...
        if (source != null && source.getItem() != null) {
            ...
            if (source.getItem() instanceof IFluidContainerItem) {
                ...
                if (targetTank != null) {
                    ...
                    ItemStack resultStack = source.copy();
                    resultStack.stackSize = 1;
                    FluidStack resultFluid = ((IFluidContainerItem) source.getItem())
                            .drain(resultStack, maxDrain, true);
                    if (resultFluid == null || resultFluid.amount <= 0
                            || (target != null && !resultFluid.isFluidEqual(target))) {
                        return new FluidAndStackResult(null, null, source, target);
                    }
     ...
     * so and put the resultant filled container item where it can go. This will also drain the tank and set it to
     * dirty.
     * 
     * <p>
     ...
     */
    public static boolean fillPlayerHandItemFromInternalTank(World world, int x, int y, int z,
            EntityPlayer entityPlayer, ITankAccess tank) {
                ...
                if (!entityPlayer.capabilities.isCreativeMode) {
                    if (fill.remainder.itemStack == null) {
                        entityPlayer.inventory
                                .setInventorySlotContents(entityPlayer.inventory.currentItem, fill.result.itemStack);
                        return true;
                    } else {
                        entityPlayer.inventory
                                .setInventorySlotContents(entityPlayer.inventory.currentItem, fill.remainder.itemStack);
                    }
    ...
    public static boolean fillInternalTankFromPlayerHandItem(World world, int x, int y, int z,
            EntityPlayer entityPlayer, ITankAccess tank) {
    ...
    public static class FluidAndStackResult {
        ...
        public FluidAndStackResult(FluidStack fluidStackResult, ItemStack itemStackResult,
                FluidStack fluidStackRemainder, ItemStack itemStackRemainder) {
