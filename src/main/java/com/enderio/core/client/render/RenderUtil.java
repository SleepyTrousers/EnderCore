...
public class RenderUtil {
    ...
    /**
     * Non-thread-safe holder for the current render pass. You must update this in your block's canRenderInPass method
    ...
    public static Matrix4d createBillboardMatrix(Vector3d lookAt, EntityLivingBase entityPlayer) {
        Vector3d playerEye = new Vector3d(
                entityPlayer.posX,
    ...
    public static void renderBillboard(Matrix4d lookMat, float minU, float maxU, float minV, float maxV, double size,
            int brightness) {
    ...
    public static void renderConnectedTextureFace(IBlockAccess blockAccess, Block block, int x, int y, int z,
            ForgeDirection face, IIcon texture, boolean forceAllEdges, boolean translateToXYZ,
            boolean applyFaceShading) {
    ...
    public static List<ForgeDirection> getNonConectedEdgesForFace(IBlockAccess blockAccess, int x, int y, int z,
            ForgeDirection face) {
        return getNonConectedEdgesForFace(blockAccess, x, y, z, face, null);
    }

    public static List<ForgeDirection> getNonConectedEdgesForFace(IBlockAccess blockAccess, int x, int y, int z,
            ForgeDirection face, IConnectedTextureRenderer render) {
        ...
        for (EdgeNeighbour edge : edges) {
            ...
            if (matchingNeighbour && render != null) {
                matchingNeighbour = render.matchesMetadata(
                        blockAccess.getBlockMetadata(x, y, z),
                        blockAccess.getBlockMetadata(edge.bc.x, edge.bc.y, edge.bc.z));
            }
    ...
    public static void getUvForCorner(Vector2f uv, Vector3d corner, int x, int y, int z, ForgeDirection face,
            IIcon icon) {
        if (icon == null) {
            return;
        }
    ...
    public static void drawBillboardedText(Vector3f pos, String text, float size, Vector4f txtCol, boolean drawShadow,
            Vector4f shadowCol, boolean drawBackground, Vector4f bgCol) {
        GL11.glPushMatrix();
