...
public class VScrollbar implements IHideable {
    ...
    public void adjustPosition() {
        ...
        btnUp = new Rectangle(
                xPosition,
                yPosition,
                (int) EnderWidget.UP_ARROW_OFF.width,
                (int) EnderWidget.UP_ARROW_OFF.height);
        btnDown = new Rectangle(
                xPosition,
                yPosition + Math.max(0, height - (int) EnderWidget.DOWN_ARROW_OFF.height),
                (int) EnderWidget.DOWN_ARROW_OFF.width,
                (int) EnderWidget.DOWN_ARROW_OFF.height);
        thumbArea = new Rectangle(
                xPosition,
                yPosition + btnUp.height,
                (int) EnderWidget.VSCROLL_THUMB_OFF.width,
        ...
        if (visible) {
            ...
            if (scrollMax > 0) {
                ...
                boolean hoverThumb = thumbArea.contains(mouseX, mouseY) && mouseY >= thumbPos
                        && mouseY < thumbPos + (int) EnderWidget.VSCROLL_THUMB_OFF.height;
